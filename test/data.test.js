...
test('assets.js data is all valid', () => {
  const w = loadGlobals(kit('assets.js'));
  // >= 1, not a bigger number: replacing the seed assets with your own game
  // must never turn the tests red.
  assert.ok(Array.isArray(w.ASSETS) && w.ASSETS.length >= 1);
...
test('rulebook.js has all sections', () => {
  ...
  for (const k of ['title', 'theme', 'howToPlay', 'aTurn', 'winCondition', 'pieces', 'ranges']) {
