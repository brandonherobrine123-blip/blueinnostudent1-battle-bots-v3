...
- `rulebook.js` — your game's rules **and its name, emoji, and tagline** (the page header comes from here).
- `gallery.js` / `gallery-core.js` / `index.html` / `styles.css` / `print.css` — the gallery renderer (you don't edit these).
- `scripts/` — optional helpers (AI art, a shape export, a key check).

## Make it yours
1. Ask your agent to rewrite `rulebook.js` for your game — the title, header, rules, and section names all come from that one file.
...
## Optional: AI art
Copy `.env.example` to `.env`, paste the class `ART_API_KEY`, then `node scripts/test-key.js`.
